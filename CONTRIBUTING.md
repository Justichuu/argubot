...
Open `argubot.js`, find the style you mean to extend, and add a matched
`for` / `against` pair that shares one rhetorical move. Run `npm test`. The
suite checks that both sides exist, that they differ, that they end in
punctuation, that ids are unique, and that the word counts stay balanced.
...
- New styles beside the ones already registered
